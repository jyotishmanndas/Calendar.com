...
export default async function AvailabilityId({ params }: { params: { availabilityId: string } }) {
        ...
        <div>
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-x-2">
                    ...
                    <h1 className="text-lg text-white font-bold">Reminder Email</h1>
                </div>
                                ...
                                <div className="flex gap-x-8">
                                    <Select name={`fromTime-${item.id}`} defaultValue={item.fromTime}>
                                        ...
                                        <SelectContent>
                                            <SelectGroup>
                                                {times.map((time) => (
                                                    <SelectItem key={item.id} value={time.time}>
                                                        {time.time}
                                                    </SelectItem>
                                                ))}
                                            </SelectGroup>
